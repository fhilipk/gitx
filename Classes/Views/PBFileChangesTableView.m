...
@property (readonly) PBGitIndexController *indexController;
@end
...
- (NSArray<PBChangedFile *> *) filesForUnstaging {
	return self.indexController.stagedFilesController.selectedObjects;
}
...
- (NSArray<PBChangedFile *> *) selectedFilesInThisTable {
	NSArrayController * relevantController = self.tag == 0 ? self.indexController.unstagedFilesController : self.indexController.stagedFilesController;
...
{
	[self.indexController moveToTrash:self.selectedFilesInThisTable];
}

- (IBAction) openFilesAction:(id)sender
{
	[self.indexController openFilesAction:self.selectedFilesInThisTable];
}

- (IBAction) ignoreFilesAction:(id)sender
{
	[self.indexController ignoreFilesAction:self.selectedFilesInThisTable];
}

- (IBAction) showInFinderAction:(id)sender
{
	[self.indexController showInFinderAction:self.selectedFilesInThisTable];
}
...
-(NSView *)nextKeyView
{
    return [self.indexController nextKeyViewFor:self];
}

-(NSView *)previousKeyView
{
    return [self.indexController previousKeyViewFor:self];
}
