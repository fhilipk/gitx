...
- (void) commitWithVerification:(BOOL) doVerify
{
	if ([[NSFileManager defaultManager] fileExistsAtPath:[repository.gitURL.path stringByAppendingPathComponent:@"MERGE_HEAD"]]) {
		[repository.windowController
		 showMessageSheet:NSLocalizedString(@"Cannot commit merges",
											@"Title for sheet that GitX cannot create merge commits")
		 infoText:NSLocalizedString(@"GitX cannot commit merges yet. Please commit your changes from the command line.",
	...
	if ([[cachedFilesController arrangedObjects] count] == 0) {
		[repository.windowController
		 showMessageSheet:NSLocalizedString(@"No changes to commit",
											@"Title for sheet that you need to stage changes before creating a commit")
		 infoText:NSLocalizedString(@"You need to stage some changed files before committing by moving them to the list of Staged Changes.",
	...
	if (commitMessage.length < kMinimalCommitMessageLength) {
		[repository.windowController
		 showMessageSheet:NSLocalizedString(@"Missing commit message",
											@"Title for sheet that you need to enter a commit message before creating a commit")
		 infoText:[NSString stringWithFormat:NSLocalizedString(@"Please enter a commit message at least %i characters long before commiting.",
